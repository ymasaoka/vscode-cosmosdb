...
export class GraphViewServer extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._httpServer.listen(
        ...
        () => {
          ...
          console.log(`** GraphViewServer listening to port ${this._port} for ${this.configuration.possibleEndpoints.join("/")}`);
          resolve();
        });
  ...
  private async _executeQueryCore(queryId: number, gremlinQuery: string): Promise<any[]> {
    if (this.configuration.actualEndpoint) {
      return this._executeQueryCoreForEndpoint(queryId, gremlin, this.configuration.actualEndpoint, this.configuration.endpointPort);
    } else {
      // We haven't figured out yet which endpoint actually works (if any - network could be down), so try them all and remember which one succeeds
      let promises = this.configuration.possibleEndpoints.map(endpoint => this._executeQueryCoreForEndpoint(queryId, gremlinQuery, endpoint, this.configuration.endpointPort));
      let finalErr = undefined;

      for (let i = 0; i < this.configuration.possibleEndpoints.length; ++i) {
        try {
          let endpoint = this.configuration.possibleEndpoints[i];
          let result = await promises[i];

          this.configuration.actualEndpoint = endpoint;
          return Promise.resolve(result);
        } catch (err) {
          if (!finalErr || err.code != "ENOTFOUND") {
            finalErr = err;
          }
        }
      }
  ...
  private async _executeQueryCoreForEndpoint(queryId: number, gremlinQuery: string, endpoint: string, port: number): Promise<any[]> {
